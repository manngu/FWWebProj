...
<tr><td>선택</td><td>번호</td><td>상품이름</td><td>옵션이름</td><td>상품수량</td><td>상품가격</td><td>옵션추가가격</td><td>총가격</td></tr>
<c:forEach var="c" items="${list}" varStatus="st">
<tr><td><input type="checkbox" name="check" value="${c.cartNo}"></td>
<td>${st.count}</td><td>${c.gdsName}</td><td>${c.optionName}</td><td>${c.cartStock}</td>
...
$(document).on("click","input[name=pay]",function(){
	...
	if(confirm_val){
		...
		$.ajax({
			url : "/mypage/carts/pay",
			type : "post",
			data : {check : checkArr},
			success : function(result){
				if(result == 1) {
					//console.log(checkArr);
					location.href = "/mypage/carts/orderinfo";
				} else {
					alert("삭제 실패");
				}
			}
