...
<body>
	...
	<form action="<c:url value='/mypage/carts/insert'/>" method="get">
		<input type="hidden" name="gdsNo"value="${info.gdsNo}">
		
		${info.gdsThumbImg } <br>
		이름 : ${info.gdsName} <br>
		가격 : ${info.gdsPrice} <br>
		수량 : <button type="button" class="minus">-</button> 
			 <input type="number" name="cartStock" class="numBox" min="1" max="5" value="1" readonly="readonly"/>
			 <button type="button" class="plus">+</button>
			 <br>
		옵션 :  <input type="hidden"name="optionNo" value="625">임의 옵션 값 625 설정<br>
		${info.gdsDes} <br><br>
					...
					<tr>
						...
						<c:if test="${user.userNo != null }">
							<td>
								...
								<c:if test="${user.userNo == L.userNo  || user.adminVerify==9}">
						...
						<form action="<c:url value='/review/delete'/>" method="post">
							...
							<tr>
								...
								<td><fmt:formatDate value="${R.reviewRegDate}"
										pattern="yyyy년 MM월 dd일 a hh:mm" /></td>
								<td></td>
								<c:if test="${user.userNo == R.userNo  || user.adminVerify==9}">
									<td>
					...
					<tr>
						...
						<c:if test="${user.userNo != null }">
							<td>
								...
								<c:if test="${user.userNo == Q.userNo  || user.adminVerify==9}">
					...
					<c:if test="${QmodifyNo != QR.qnaNo}">
						<form action="<c:url value='/qna/delete'/>" method="post">
							...
							<tr>
								...
								<td><fmt:formatDate value="${QR.qnaRegDate}"
										pattern="yyyy년 MM월 dd일 a hh:mm" /></td>
								<c:if test="${user.userNo == QR.userNo  || user.adminVerify==9}">
									<td>
